...
const action = async (
  ...
  const { audiences, thread: threadId } = props;
  ...
  await openai.beta.threads.messages.create(
    ...
    {
      role: "user",
      content: `I have the following audience descriptions:\n ${
        audiences.map(formatAudience).join("\n")
              ...
              "items": {
                ...
                "properties": {
                  "name": {
                    "type": "string",
                    "description":
                      "Individual full name, e.g. John Luke Laplace",
                  },
                  "age": {
                    "type": "number",
...
const cache = await caches.open("tmp_aliens");
...
export default async (
  ...
  try {
    ...
    url.searchParams.set("audience", JSON.stringify(props.audiences));
